...
import gym
import text_crafter.text_blockmaze
...
inventory_keys_ordered = []
actions_sorted = [ 'move_down', 'move_left',
              'move_right', 'move_up']
objects = []
class Predictor():
...
def convert_hard_act_to_simple(hard_act):
    ...
    if hard_act.replace(' ', '_') in ['move_up', 'move_down', 'move_left', 'move_right']:
